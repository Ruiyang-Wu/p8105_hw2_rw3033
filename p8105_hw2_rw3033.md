...
Describe your data cleaning process: After importing the three datasets,
I first checked for missing (NA) values. I ensured that baker names were
in a consistent format by using janitor::clean_names() and then
separated the full names in bakers.csv into baker and baker_lastname to
match the format in bakes.csv and results.csv. Next, I identified bakers
present in bakes.csv and results.csv but missing in bakers.csv and
filtered out these entries. Finally, I merged the three datasets into
combined_df and verified that all rows were distinct to ensure data
integrity. The final dataset (combined_df) contains all the information,
which results in some duplicated entries. However, I decided to keep it
this way to ensure it includes the most comprehensive information about
the show for future use. If specific details are needed, I can extract
...
```
    ...
    ## # A tibble: 10 × 6
    ##    episode result     `5`     `6`    `7`       `8`   
    ##      <dbl> <chr>      <chr>   <chr>  <chr>     <chr> 
    ##  1       1 STAR BAKER Nancy   Marie  Jane      Steven
    ##  2       2 STAR BAKER Richard Ian    Candice   Steven
    ##  3       3 STAR BAKER Luis    Ian    Tom       Julia 
    ##  4       4 STAR BAKER Richard Ian    Benjamina Kate  
    ##  5       5 STAR BAKER Kate    Nadiya Candice   Sophie
    ##  6       6 STAR BAKER Chetna  Mat    Tom       Liam  
    ##  7       7 STAR BAKER Richard Tamal  Andrew    Steven
    ##  8       8 STAR BAKER Richard Nadiya Candice   Stacey
    ##  9       9 STAR BAKER Richard Nadiya Andrew    Sophie
    ## 10      10 WINNER     Nancy   Nadiya Candice   Sophie

Comment on the table: The table presents the Star Baker and Winner
results for Episodes 1 through 10 across Seasons 5 to 8. Each column
...
The final row displays the overall Winner of each season. The table
shows some predictable winners, like Nadiya in Season 6 and Candice in
Season 7, who were Star Baker multiple times. However, there were
surprises too, such as Nancy winning Season 5 despite Richard earning
Star Baker five times, and Sophie winning Season 8 over Steven, who had
more Star Baker titles. Consistency doesn’t always translate to victory,
making the final outcomes less predictable in some seasons.
